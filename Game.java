...
public class Game extends WindowController {
    ...
    public void begin() {
        ...
        //setting board frame
        FramedRect frame = new FramedRect(0,0,size*8,size*8,canvas);
        ...
        for (int row = 1; row <= 8; row++) {
            for (int col = 1; col <= 8; col++) {
                ...
                board[row][col].setColor(Color.BLACK, Color.WHITE);
            }
        }
        ...
        for (int t = 0; t < pieces.length; t++){
            for (int n = 0; n < pieces[t].length; n++){

                Piece temp = pieces[t][n];
                if (temp.isGrey()){

                    board[temp.getRow()][temp.getCol()].setValue(2);
                }
                else {
                    board[temp.getRow()][temp.getCol()].setValue(1);
    ...
    //onMousePress and onMouseRelease will be the two main methods, and the program will react differently depending on where and when the mouse is pressed and released
                ...
                for (int col = 1; col <= 8; col++) {
                    if (board[row][col].contains(point)) {
                        ...
                        if (pieces[turn][selectedPiece].isValidMove(board[row][col], board)) {
                            ...
                            pieces[turn][selectedPiece].moveTo(board, row, col);
