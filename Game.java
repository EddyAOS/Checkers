...
public class Game extends WindowController {
    ...
    public void onMouseDrag(Location point) {
        ...
        if (pieceSelect) {
            ...
            pieces[turn][selectedPiece].getKingCircle().move( point.getX() - lastPoint.getX(), point.getY() - lastPoint.getY() );
            lastPoint = point;
        }
    }
            ...
            for (int row = 1; row <= 8; row++) {
                for (int col = 1; col <= 8; col++) {
                    if (board[row][col].contains(point)) {
                        ...
                        if (pieces[turn][selectedPiece].isValidMove(board[row][col], board, true)) {
                            ...
                            if (!pieces[turn][selectedPiece].hasValidCapture(board)) {
                                if (turn == 0) turn++;
                                else turn--;
                            }
